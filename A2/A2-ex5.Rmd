...
First, we make a graphical summary of the data by plotting each variable against the others. Furthermore, we construct a histogram of all the numerical variables. Looking at the plots, we observe that expend, lawyers, employ and pop all approximate a linear relationship with all the other variables. Furthermore, state and crime have nonlinear relationhsips with all the other variables. Lastly, the variable bad can be argued to have a weak linear relationship with the variables expend, lawyers, employ and pop. Looking at the histograms, it is interesting to see that almost all variables (expend, bad, lawyers, employ and pop) follow a similar pattern, namely, the lowest value appears frequently and as the value increases, the frequency decreases steeply. Except for a few outliers of frequently occuring high values. In contrast, the variable crime shows a different pattern. Namely, the values in the middle occur also relatively frequently. But the rule: as the value increases, the frequency decreases, applies as well.
<!-- Scatter plots -->
...
#3 Added variable plot of residuals of Xj against residuals of Y with omitted Xj
par(mfrow=c(2,3))
x = residuals(lm(bad~employ+crime))
y = residuals(lm(expend~employ+crime))
plot(x,y,main="Added variable plot for bad")
x = residuals(lm(crime~bad+lawyers+pop))
y = residuals(lm(expend~bad+lawyers+pop))
plot(x,y,main="Added variable plot for crime")
x = residuals(lm(lawyers~employ+crime))
y = residuals(lm(expend~employ+crime))
plot(x,y,main="Added variable plot for lawyers")
x = residuals(lm(employ~bad+lawyers+pop))
y = residuals(lm(expend~bad+lawyers+pop))
plot(x,y,main="Added variable plot for employ")
x = residuals(lm(pop~employ+crime))
y = residuals(lm(expend~employ+crime))
