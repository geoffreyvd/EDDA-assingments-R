...
tstar=numeric(B)
for (i in 1:B) {
  medicationstar=sample(medication) # permute medicine lables
    tstar[i] = chisq.test(xtabs(~patientdata$medicine+patientdata$nausea))[[1]] }
test_statistic=mystat(lm(patientdata$nausea~patientdata$medicine));test_statistic
hist(tstar)
pl=sum(tstar<myt)/B
pr=sum(tstar>myt)/B
pl
...
```{r}
chisq.test(xtabs(~nausea_table$medicine+nausea_table$nausea))
```
