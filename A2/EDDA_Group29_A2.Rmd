...
**c)** <!-- Compare the p-value found by the permutation test with the p-value found from the chisquare test for contingency tables. Explain the difference/equality of the two p-values. --> When performing the chi-squared test, we observe that the p-value is equal to 0.03643. This is smaller than the significance level of 0.05, therefore, $H_0$ (the medicines work equally well against nausea) should be rejected. This is a similar outcome to the permutation test. However, the p-values do differ. The values lie close to eachother as both methods are valid tests in our situation. The difference can be explained by the different manner of calculating the p-value.
