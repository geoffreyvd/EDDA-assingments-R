...
We fit a logistic regression model that explains if a students did psi based on their gpa and test score. We test the null hypotheses that doing psi does influence the gpa and test score. 

```{r}
glm = glm(psi~passed+gpa,data=data,family=binomial)
summary(glm)
...
```{r}
tot=xtabs(~psi+gpa,data=data);tot
per=xtabs(passed~psi+gpa,data=data);per
round(per/tot,2)
```
